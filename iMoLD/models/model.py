...
class Separator(nn.Module):
    def __init__(self, args, config):
        ...
        if args.dataset.startswith('GOOD'):
            ...
            if 'graph-' in  args.dataset.lower():
                in_dim = 768
            else:
                in_dim = 39
            self.r_gnn = GNN_node(num_layer=args.layer, emb_dim=args.emb_dim,
                                  drop_ratio=args.dropout, gnn_type=args.gnn_type, in_dim=in_dim)
            emb_d = args.emb_dim
...
class DiscreteEncoder(nn.Module):
    def __init__(self, args, config):
        ...
        if args.dataset.startswith('GOOD'):
            ...
            self.gnn = GNN_node(num_layer=args.layer, emb_dim=args.emb_dim,in_dim=in_dim,
