...
import numpy as np
# from good_utils.evaluation import eval_data_preprocess, eval_score
# from good_utils.metric import Metric
# from good_utils.train import nan2zero_get_mask
...
from dataset.graphss2_dataset import get_dataloader_per, get_dataset
...
class Runner:
    def __init__(self, args, writer, logger_path):
        ...
        elif args.dataset.lower().startswith('graph-'):
            dataset = get_dataset(dataset_dir='./data/', dataset_name=args.dataset, task=None)
            dataloader,train_dataset,val_dataset,test_dataset \
            = get_dataloader_per(dataset, batch_size=args.bs, small_to_large=False, seed=args.random_seed,return_set=True)
            print (train_dataset[0])
            self.train_loader = dataloader['train']
            self.valid_loader = dataloader['eval']
            self.test_loader = dataloader['test']
            self.metric = Metric()
            self.metric.set_loss_func(task_name='Multi-label classification')
            self.metric.set_score_func(metric_name='Accuracy')
            cfg = Munch()
            cfg.metric = self.metric
            cfg.model = Munch()
            cfg.model.model_level = 'graph'
            self.save_dir = f"results/{args.dataset}/numE_{args.num_e}_commitmentWeight_{args.commitment_weight}_invW_{args.inv_w}_regW_{args.reg_w}_gamma_{args.gamma}/"
            print ('args')
            print (args)
            print ('cfg:')
            print (cfg)
    ...
    def run(self):
        ...
        write_results_to_file(self.save_dir, f'test_score_seed_{self.seed}.txt', f"test score is: {best_test_score}")
